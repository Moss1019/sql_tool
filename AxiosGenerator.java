...
public class AxiosGenerator {
  ...
  private String generateDelete(Table t) {
    ...
    if(t.isJoiningTable()) {
      ...
      if(t.hasJoiningTable()) {
        ...
        b
        .append(generateExportCode(parameterBuilder.toString(), 
        String.format("delete%ss", methodNameBuilder.toString()),
        urlBuilder.toString()));
      }
      b
  ...
  private String generateGetParentChildren(Table t) {
    StringBuilder b = new StringBuilder();
    if(t.isJoiningTable()) {
      Table p = Table.getJoiningPrimary(t);
      Table s = Table.getJoiningSecondary(t);
        b
        .append(generateExportCode(p.getPrimaryColumn().getPascalName(), 
        String.format("get%ssFor%s", t.getCleanName(), p.getCleanName()),
        String.format("%ss/${%s}", t.getName(), p.getPrimaryColumn().getPascalName())))
        .append(getAxiosReturnString("get", null))
        .append("}\n\n");
    } else {
